...
pub fn extract_sni(buf: &[u8]) -> Option<String> {
    if !(tls_client_hello(buf)) {return None;}
    ...
    if index + 1 + session_id_len > buf.len() {return None;}
    ...
    while index < extension_len_end_index {
        ...
        if ext_type == 0 {
            ...
            if sni_start + sni_name_len > buf.len() {break;}

            let sni =String::from_utf8_lossy(&buf[sni_start..sni_start + sni_name_len]).to_string();
